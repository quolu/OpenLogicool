...
/// <summary>
/// G600 の live adapter。message-only window で vendor page 0xFF80 の Raw Input を受信し、
/// VID/PID で G600 だけへ絞って G600ReportStream（recorded adapter と同一経路）で変換する。
...
public sealed class G600RawInputSource : IDeviceInputSource, IDeviceChangeSource, IG600WheelSource, IDisposable
{
    ...
    public G600RawInputSource()
    {
        wndProc = WndProc;
        pumpThread = new Thread(MessagePump) { IsBackground = true, Name = "G600RawInput" };
        pumpThread.Start();
        pumpReady.Wait();
        if (pumpFailure is not null)
        {
            throw new InvalidOperationException("G600 Raw Input の受信窓を初期化できませんでした。", pumpFailure);
        }
    }
    ...
    private void ThrowIfPumpFailed()
    {
        if (pumpFailure is not null)
        {
            throw new InvalidOperationException("G600 Raw Input の受信 thread が停止しています。", pumpFailure);
        }
    }
    ...
    private void MessagePump()
    {
        try
        {
            var className = $"OpenLogicoolG600RawInput-{Environment.CurrentManagedThreadId}";
            ...
            windowHandle = CreateWindowEx(0, className, "g600", 0, 0, 0, 0, 0, HWND_MESSAGE, IntPtr.Zero, wndClass.hInstance, IntPtr.Zero);
            if (windowHandle == IntPtr.Zero)
            {
    ...
    private void HandleRawInput(IntPtr rawInputHandle)
    {
        ...
        try
        {
            ...
            if (hid.SizeHid != G600DeviceIdentity.InputReportLength)
            {
                return;
            }
            ...
            for (var i = 0; i < hid.Count; i++)
            {
                ...
                feedBuffer.Clear();
                stream.Feed(report, elapsedMs, feedBuffer, out var wheelTick);
                ...
                {
                    if (wheelTicks.Count >= MaxQueuedWheelTicks)
                    {
                        wheelTicks.TryDequeue(out _);
                    }
