...
### route 最終決定（オーナー裁定 2026-08-15: B主・A補完・C不採用で確定）

下表の推奨どおり確定した。**方式B変種が主経路**（side 12ボタンを F13〜F24 へ書換えて legacy 配送を無害化）、**方式Aは補完**（Input Studio 管理 slot ⇔ 素の slot の一括切替・退避）、**方式Cは不採用のまま最後の手段**（G1〜G5 を標準マウスとして残す設計では不要。採否再考は計画 §16「optional kernel driver」の期限に従う）。
