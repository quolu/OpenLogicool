...
| exploration pointer／click route | **GameLabのstandard integrity SendInputは実測成立（2026-08-24）**。pointer移動、frame-bound click、Escape、scroll、generic F13をreceiver-sideまたは画面before／afterで確認。NIKKE lobbyは未確認 | EXP-GS-04。対象gameで個別実測するまでlive Exploreは禁止し、別routeへfallbackしない。証跡: `evidence/phase9-game-structure-discovery/t05-discovery-admission.md` |
