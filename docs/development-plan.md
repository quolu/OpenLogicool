...
| SendInput acceptance（EXP-IN-01） | Phase 2内。**standard 側は実測完了（2026-08-16）: 確認済み**——受信側観測（probe `sendinput-accept`・test target window の WM_KEYDOWN/KEYUP log と送信列の突合・Windows 11 26200）で、standard foreground window への keyboard 配送が単一 key・chord とも完全順序一致（6/6）。elevated 側と target game 分類は未実測（elevated target の起動に UAC 承認＝オーナー手番が必要）。mouse button 分類は cursor 依存のため未実施 | 証跡: probe-output/sendinput-accept-standard-20260816-014738-596.json。elevated 実測は watchdog の昇格/uiAccess 採否（上行）と同時に判定する |
