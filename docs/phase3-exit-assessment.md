...
### 条件2: OpenLogicool へ Alt+Tab しても editing target を失わない — **強い推定**

- 構造: 編集対象 workspace は上部 pill の明示選択だけで変わる。foreground 追跡（200ms poll）は runtime profile 切替専用で、編集画面の状態へ到達する経路がない（MAP-010 系の分離）。
- 実測: foreground 往復下の runtime 切替 log は p1-core live run で確認済みだが、「UI 編集中に Alt+Tab して戻る」操作そのものの目視実測は未記録。オーナー裁定時に1操作で確認可能（下記「オーナー手番」）。

### 条件3: launcher・同名 EXE・window 消失を誤 profile で継続しない — **強い推定**

- 確認済み部分（p1-core live run・[evidence/phase3-app-first/p1-core-live-run.md](../evidence/phase3-app-first/p1-core-live-run.md)）: EXE full path 完全一致切替（explorer）・package identity 一致切替（Store メモ帳・APP-004）・往復の理由付き log（APP-005）・昇格 process で誤 Unknown なし（APP-008）。window 消失（ForegroundState 3状態・取得不能は既定へ）は focused test で検証済み。
- 未確認部分: NIKKE launcher→本体遷移の実測・Unknown 実トリガーの実測（p1-core からの持ち越し。オーナー手番）。
...
## オーナー手番（Exit 裁定時に確認を依頼する残項目）

1. UI 編集中に Alt+Tab で他アプリへ→戻って編集対象が保持されていること（条件2の実測化）
2. NIKKE launcher→本体遷移で誤 profile 継続がないこと（条件3の残実測）
3. 実機接続下の `ui --resident` で保存→即時反映（t09 持ち越し）
4. キー録画 modal の対話操作目視（t09 持ち越し）

## 判定要旨

Exit 5条件は 確認済み3（条件1・4[G600]・5）／強い推定2（条件2・3——いずれも構造成立・残るは上記オーナー手番の実測のみ）。未確認・非対応の条件はない。最終 Exit 宣言はオーナー裁定。
