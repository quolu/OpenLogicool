...
現在の開発配布 layout は unpackaged であり、`OpenLogicool.Launcher.exe`、`OpenLogicool.Host.exe`、`OpenLogicool.Watchdog.exe`、各exeの`.dll`／`.deps.json`／`.runtimeconfig.json`とruntime dependenciesを同じapplication配下に置く。`scripts/install-development-app.ps1`がこのlayoutを`artifacts/development/OpenLogicool/`へ再現し、デスクトップショートカットをconsoleなしのLauncherへ向ける。
