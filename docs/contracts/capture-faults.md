...
WGC は `SystemRelativeTime`（compositor が描画した QPC 時刻）を source 内の arrival clock へ対応付け、`FreshnessMs` に描画から取得までの frame age を入れる。画素 fingerprint の最終変化からの QPC 経過は `LastChangeMs`（内容が変わらず続いた時間）へ入れる。静止して新 frame 自体が供給されない場合はこの値を更新せず、`FrameUnavailable` は依然として fault ではない。
