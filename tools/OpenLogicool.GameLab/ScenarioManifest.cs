...
public static class ScenarioManifestLoader
{
    ...
    {
        var manifest = JsonSerializer.Deserialize<ScenarioManifest>(File.ReadAllText(filePath), JsonOptions)
            ?? throw new InvalidDataException($"scenario manifest '{filePath}' could not be read.");

        if (manifest.Schema != "scenario-v1")
        {
            throw new InvalidDataException($"scenario manifest '{filePath}' has unsupported schema '{manifest.Schema}'.");
        }
