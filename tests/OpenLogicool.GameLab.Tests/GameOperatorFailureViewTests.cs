...
public sealed class GameOperatorFailureViewTests
{
    ...
    {
        var messages = GameOperatorFailureView.Project(new(
            new CaptureFault(CaptureFaultKind.Occluded, "window hidden"),
    ...
    public void Non_known_observation_is_visible_and_not_an_automatic_condition(
        CaptureAvailability availability,
        StateIdentityStatus identity)
    {
        var messages = GameOperatorFailureView.Project(new(null, availability, identity, false, VerificationLevel.Confirmed));
    ...
    {
        var messages = GameOperatorFailureView.Project(new(
            null, CaptureAvailability.Available, StateIdentityStatus.Known, true, VerificationLevel.StrongInference));
