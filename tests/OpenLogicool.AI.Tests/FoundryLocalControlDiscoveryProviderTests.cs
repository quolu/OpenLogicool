...
public sealed class FoundryLocalControlDiscoveryProviderTests
{
    ...
    public async Task Target_intent_limits_visual_discovery_to_one_goal_control()
    {
        ...
        Assert.False(result.ProviderResult.Normalization.HasFlag(FoundryVisionNormalization.TargetIntentMismatchDropped));
        Assert.Equal(client.ControlsPromptSha256For("ホームへ戻る"), result.Scene.DiscoveryEvidence!.PromptSha256);
    }

    [Fact]
    public async Task Model_selected_intermediate_control_is_kept_without_client_text_gate()
    {
        using var client = ClientReturning(
        ...
        Assert.Equal("アーツ", Assert.Single(result.Scene.Affordances).SemanticLabel);
