...
public sealed class FrozenMetricRunnerTests
{
    ...
    public void Acceptance_fixtures_run_through_recognizer_and_observation()
    {
        ...
        var cases = new[] { Case(known, StateIdentityStatus.Known, true), Case(unknown, StateIdentityStatus.InsufficientEvidence, false) };
    ...
    public void Actual_fixture_misclassification_fails_all_relevant_fixed_metrics()
    {
        ...
        var item = Case(frame, StateIdentityStatus.InsufficientEvidence, false);
    ...
    private static FrozenMetricCase Case(CapturedFrame frame, StateIdentityStatus expected, bool dispatch) =>
        new(new CorpusArtifact(frame.SourceId, $"acceptance/{frame.SourceId}.png", "fixture:phase5"), frame,
            CaptureAvailability.Available, expected, dispatch);
